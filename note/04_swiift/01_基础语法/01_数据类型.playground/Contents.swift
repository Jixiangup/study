...
/*
 ...
 2. 在下面的所有方式中都是隐式创建，也就是说系统自动识别了数据类型
 */
// 输出语句
//print("Hello，Bnyte！")
...
/*
 3. 下面所有方式使用显示声明变量类型的方式声明并且创建变量
    3.1 print(amount + price) 71.89999999999999
        需要注意的是在swift中的浮点型依然存在京都丢失问题，但是当使用显示数据类型隐式数据类型去做数据的运算并不会报错，这是可以存在的
    3.2 print(name + amount) 报错（Binary operator '+' cannot be applied to operands of type 'String' and 'Double'）
    3.3 print(price + age) 报错（Binary operator '+' cannot be applied to operands of type 'Double' and 'Int'
        总结3.2、3.3：也就是说在swift中，二元运算符不能作用于不同的数据类型
    3.4 print(name + password) // bnytebnyte.. 和Java一样，相同类型是可以进行运算的，如果是字符串则会拼串，而如果是数值类型则会进行数学运算
    3.5 `字符串拼接`功能：当需要使用字符串以及变量数据进行拼接时，可以直接使用反斜线加括号（`\()`）的方式进行拼接，括号中可以直接输入变量名，这样就可以完成字符串拼接，并不会引起运算出现的问题
    3.6 多行字符串：
        语法："""字符串内容体"""
